<template >
  <div class="shop-list-big-box" style="padding:20px;">
    <div class="top-content-box">
      <div class="title-box">
        <div class="left-text">
          <p class="title">项目列表</p>
          <el-button type="primary" size="mini" icon="el-icon-plus" @click="addClassfifyShop">添加项目</el-button>
          <!--<el-button size="small" type="primary" icon="el-icon-plus">添加项目</el-button>-->
        </div>
        <div class="right-text">
          <el-select size="small" v-model="shopType" placeholder="筛选项目类型" @change="shopTypeChange">
            <el-option label="全部" :value="0">全部</el-option>
            <el-option label="普通项目" :value="1">普通项目</el-option>
            <el-option label="拼团项目" :value="2">拼团项目</el-option>
            <el-option label="集赞项目" :value="3">集赞项目</el-option>
          </el-select>
        </div>
      </div>
      <!-- 筛选 -->
      <div class="screen-box">
        <div class="line-screen-box">
          ...
          <div class="select-input name-input-margin">
            <el-select size="small" v-model="type" placeholder="请选择项目类别">
        ...
        <el-table
          ...
          <el-table-column label="项目价格">
            <template slot-scope="scope">
              ...
              <!--<span style="color: rgb(255, 132, 0);">{{'￥'+scope.row.price}}</span>-->
            </template>
          </el-table-column>
          ...
          <el-table-column label="项目佣金">
            <template slot-scope="scope">
              <span>{{'￥'+scope.row.goods_reward}}</span>
            </template>
          </el-table-column>
    ...
    <el-dialog title="项目详情" :visible.sync="dialogSwiperVisible" width="600px" top="20vh">
  ...
  export default {
    ...
    methods: {
      ...
      getShopList(tip){
        ...
        this.$store.dispatch('ShopLists', data).then((res) => {
          // console.log(res);
      ...
      saleSave(val, index, goods_id){
        let reg = /^[0-9]{1,8}$/;
        if(reg.test(val)){
          this.$store.dispatch('EditShopSales', {
            token: true,
            goods_id: goods_id,
            sales: val
          }).then((res) => {
      ...
      // 前往添加项目页面
      addClassfifyShop(){
        this.$router.push({path: '/shop/addEdit'});
      },
      // 显示项目详情轮播图
      ...
      operation(type, index, id){
        let _this = this;
        if(type==0){ // 编辑
          this.$router.push({path: '/shop/addEdit', query: { goods_id: this.tableData[index].goods_id }});
        }else if(type==1){ // 删除
          let goods_id = '';
          let info = '';
          if(id){
