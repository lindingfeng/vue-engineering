<template >
  <div class="add-classfify-box">
    <div class="add-classfify-title">添加分类</div>
    <div class="base-info-form">
      <el-form ref="form" :model="form" label-width="120px">
        <el-form-item label="科目类型" prop="parent" :rules="{required: true, message: '科目类型不能为空', trigger: ['blur', 'change']}">
          <el-select size="mini" v-model="form.parent" placeholder="请选择所属科目类型" style="width: 240px;">
            ...
            if(this.form.logo){
              ...
              this.$store.dispatch('SetClassfify', data).then((res) => {
                ...
                if(res.data.errcode==0){
                  ...
                  setTimeout(()=>{
                    this.$router.push({ path: '/shop/classfify/firstLists' })
                  }, 500)
                }else {
                  this.$message({
                    message: res.data.errstr+'（'+res.data.errcode+'）',
