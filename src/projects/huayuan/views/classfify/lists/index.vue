        ...
        <el-table
          ...
          <el-table-column label="操作" fixed="right">
            <template slot-scope="scope">
              ...
              <span v-else style="margin: 0 5px;color: #CCCCCC;cursor: pointer;">删除</el-popover>
              </span>
              <span v-if="scope.row.delete_state!=1" style="color: #008FFE;margin: 0 5px;cursor: pointer" @click="operation(2, scope.$index)">查看子分类</span>
              <span v-else style="color: #CCCCCC;margin: 0 5px;cursor: pointer;">查看子分类</span>
            </template>
          </el-table-column>
        </el-table>
      </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      // 获取分类列表
      getClassfifyList(){
        this.$store.dispatch('GetClassfifyPage', {
          token: true,
      ...
      operation(type, index){
        ...
        }else if(type==2){ // 查看子分类
          this.$router.push({ path: '/shop/classfify/secondLists', query: { class_id: this.tableData[index].class_id } })
        }
